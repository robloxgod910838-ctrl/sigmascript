...
pcall(function()
	PlacementHelper = require(RS.Modules.Shared.PlacementHelper)
	CharactersInfo = require(RS.Modules.Shared.CharactersInfo)
end)
...
local function waitForGame()
	local t0 = os.clock()
	while plr:GetAttribute("DataLoaded") ~= true and os.clock() - t0 < 30 do
		task.wait(0.2)
	end
end
...
local function initDataClient()
	...
	local ok, client = pcall(function()
		local c = require(RS.Data.DataService).client
		c:waitForData()
		return c
	end)
...
local function getPlayerBase()
	...
	for _, plot in plots:GetChildren() do
		...
		if owner == plr.Name or owner == plr.DisplayName or plot:GetAttribute("OwnerUserId") == plr.UserId then
			return plot
		end
	end
	return nil
...
local function firePrompt(prompt, hold)
	...
	local ok = pcall(function()
		fireproximityprompt(prompt, hold or prompt.HoldDuration)
	end)
	return ok
end
...
local function findPrompt(base, promptName, actionText)
	...
	for _, d in base:GetDescendants() do
		if d:IsA("ProximityPrompt") then
			if promptName and d.Name == promptName then
				return d
			end
			if actionText and d.ActionText == actionText then
				return d
			end
		end
	end
...
local function getShopCharacters(base)
	base = base or getPlayerBase()
	if not base then
		return {}
	end
...
local function isCharacterToolName(name)
	if not CharactersInfo or not CharactersInfo.Characters then
		return false
	end
...
local function getInventoryTools()
	...
	local function scan(container)
		...
		for _, item in container:GetChildren() do
			if item:IsA("Tool") and isCharacterToolName(item.Name) then
	...
	if fighters and PlacementHelper then
		for _, fighter in fighters:GetChildren() do
			if fighter:IsA("Model") then
				local part = fighter.PrimaryPart or fighter:FindFirstChild("HumanoidRootPart")
				if part then
...
local function buildPlacePayload(tool, cellName, cell)
	if not PlacementHelper then
		return nil
	end
	...
	local shapeModel = PlacementHelper.GetShapeModel(Shapes, tool.Name, shapeName)
	if not shapeModel then
		return nil
	end
	local ghost = shapeModel:Clone()
	ghost.Parent = Workspace
	PlacementHelper.SnapShapeToCell(ghost, cell, 0)
...
local function tryAutoPlace()
	local now = os.clock()
	if now - State.lastPlaceAt < PLACE_DELAY then
		return false
	end
	local base = getPlayerBase()
...
local function tryStartWave()
	...
	local now = os.clock()
	if now - State.lastWaveAt < WAVE_DELAY then
		return false
	end
...
local function readStats()
	local base = getPlayerBase()
	return {
		cash = tostring(getData("Cash", "?")),
		rolls = tostring(getData("Rolls", "?")),
		grid = tostring(getData("GridLevel", "?")),
		wave = base and tostring(base:GetAttribute("WaveStarted")) or "?",
		shop = tostring(#getShopCharacters(base)),
		tools = tostring(#getInventoryTools()),
		base = base and base.Name or "No plot",
	}
end
...
local function buildUI()
	...
	ver.Size = UDim2.fromOffset(200, 44)
	ver.Position = UDim2.fromOffset(210, 0)
	...
	UI.footerStatus.Size = UDim2.new(1, -16, 0, 24)
	UI.footerStatus.Position = UDim2.new(0, 8, 1, -24)
	...
	local function selectNav(name)
		for n, btn in pairs(navItems) do
			btn.BackgroundColor3 = n == name and C.cardInner or C.card
			btn.TextColor3 = n == name and C.accent or C.muted
		end
		for n, page in pairs(pages) do
			page.Visible = n == name
		end
	...
	local function sectionCard(parent, titleText)
		...
		Instance.new("UIPadding", card).PaddingTop = UDim.new(0, 12)
		Instance.new("UIPadding", card).PaddingBottom = UDim.new(0, 12)
		Instance.new("UIPadding", card).PaddingLeft = UDim.new(0, 12)
		Instance.new("UIPadding", card).PaddingRight = UDim.new(0, 12)
		local heading = Instance.new("TextLabel")
		heading.BackgroundTransparency = 1
	...
	UI.vRolls = statRow(statsInner, "Rolls", C.accent2)
	UI.vGrid = statRow(statsInner, "Grid Level", C.yellow)
	UI.vWave = statRow(statsInner, "Wave Active", C.text)
	...
	UI.logLabel = Instance.new("TextLabel")
	UI.logLabel.BackgroundTransparency = 1
	UI.logLabel.Size = UDim2.new(1, 0, 0, 130)
	UI.logLabel.Font = Enum.Font.Code
	UI.logLabel.TextSize = 11
	UI.logLabel.TextXAlignment = Enum.TextXAlignment.Left
	UI.logLabel.TextYAlignment = Enum.TextYAlignment.Top
